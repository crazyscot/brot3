...
pub(super) fn render(constants: &FragmentConstants, point: Vec2) -> RenderData {
    ...
    match constants.algorithm {
        Algorithm::Mandelbrot => Builder {
            constants,
            fractal: Mandelbrot { c: point.into() },
        }
        .iterations(),
        Algorithm::Mandeldrop => Builder {
            constants,
            // Same algorithm but add the inverse of c each time
            fractal: Mandelbrot {
                c: Complex::from(point).recip(),
            },
        }
        .iterations(),
    }
